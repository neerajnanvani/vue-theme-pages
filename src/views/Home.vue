  ...
  <div class="flex flex-col min-h-screen min-w-screen">
    <div class="">
      ...
      >
            ...
            <thead class="sticky top-16 z-20">
              <tr>
                <th scope="col" 
                  ...
                  dark:bg-gray-800 dark:text-gray-200 text-gray-500 uppercase tracking-wider">
                  S.no
                </th>
                <th
                  ...
                  :key="header"
                  scope="col"
                  class=" top-0 px-6 py-3 max-w-10 text-left text-xs font-medium bg-gray-200
                  dark:bg-gray-800 dark:text-gray-200 text-gray-500 uppercase tracking-wider"
                >
                  {{ header }}
                </th>
            ...
            <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-600">
              ...
              <template v-else>
                <tr v-for="(data, index) in tableData" :key="index">
                  ...
                  <td class="px-6 py-4 max-w-10 text-sm text-gray-500 dark:bg-gray-700 dark:text-white underline underline-offset-4 hover:text-gray-900 font-semibold">
                    <a :href="data.Link">
                      {{ data.API }}
                    </a>
                  </td>
                  <td class="px-6 py-4 max-w-20 text-sm text-gray-500 dark:bg-gray-700 dark:text-gray-100">
                    {{ data.Description }}
                  </td>
                  <td class="px-6 py-4 max-w-10 text-sm text-gray-500 dark:bg-gray-700 dark:text-gray-100">
                    {{ data.Auth }}
                  </td>
                  <td class="px-6 py-4 max-w-10 text-sm text-gray-500 dark:bg-gray-700 dark:text-gray-100">
                    {{ data.HTTPS }} / {{ data.Cors }}
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-500 dark:bg-gray-700 dark:text-gray-100">
                    {{ data.Category }}
                  </td>
                </tr>
              </template>
        ...
        <button :disabled="currentPage == 1" :class="[currentPage == 1 ? 'bg-gray-200 text-gray-700 border-1 dark:bg-gray-900 dark:border-0 dark:text-gray-400' :
            'bg-blue-100 text-blue-900 hover:border-blue-600 dark:bg-gray-700 dark:text-gray-100', 'px-3 py-2 flex items-center rounded-md border-2']"
          @click="currentPage = currentPage - 1">
          <ChevronLeft class="w-4 mr-2" />
          Previous
        </button>
        <button :disabled="!isNextPageAvailable" :class="[isNextPageAvailable ? 'bg-blue-100 text-blue-900 hover:border-blue-600 dark:bg-gray-700 dark:text-gray-100' :
            'bg-gray-200 text-gray-700 border-1 dark:bg-gray-900 dark:border-0 dark:text-gray-400', 'px-3 py-2 flex items-center rounded-md border-2']"
          @click="currentPage = currentPage + 1">
          Next
